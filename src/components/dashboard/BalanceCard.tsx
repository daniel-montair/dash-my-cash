import { useFinance } from '../../contexts/FinanceContext'
import { formatCurrency } from '../../utils/format'
import { useCountAnimation } from '../../hooks/useCountAnimation'

/**
 * BalanceCard - Card de Saldo Total
 * Fundo preto com círculo verde-limão desfocado decorativo
 * Mostra saldo total com animação de contagem
 * Badge com crescimento percentual comparado ao mês anterior
 */
function BalanceCard() {
  const { calculateTotalBalance } = useFinance()

  // Calcular saldo atual
  const currentBalance = calculateTotalBalance

  // Animação de contagem
  const animatedValue = useCountAnimation(currentBalance, 800)

  return (
    <div className="bg-[var(--white)] border border-[var(--gray-200)] rounded-[var(--border-radius-lg)] p-6 shadow-[var(--shadow-sm)]">
      {/* Header com ícone de cifrão preto no topo */}
      <div className="mb-5">
        <div className="w-10 h-10 flex items-center justify-center mb-3">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={2}
            stroke="currentColor"
            className="w-6 h-6 text-[var(--black)]"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
            />
          </svg>
        </div>
        
        {/* Label */}
        <p className="text-[var(--font-size-body)] font-[var(--font-weight-bold)] text-[var(--black)] mb-2">
          Saldo total
        </p>
      </div>

      {/* Valor formatado - em azul vibrante */}
      <p className="text-4xl font-[var(--font-weight-bold)] text-[var(--color-primary-blue)] leading-tight">
        {formatCurrency(animatedValue)}
      </p>
    </div>
  )
}

export default BalanceCard
